...
import bot.seven.WLR.utils.Extensions.getVelocity
import bot.seven.WLR.utils.Extensions.scale
...
object WorldUtils {
    ...
    fun entityOffEdge(player: EntityPlayer): Boolean {
        if (!player.onGround) {
            ...
            for (position in positions) {
                if (wlr.mc.theWorld.getBlockState(BlockPos(position)).block != Blocks.air) {
                    return false
                }
            }
            return true
    ...
    private fun airCheck(player: EntityPlayer, pos: BlockPos, distance: Float, lookVec: Vec3): Boolean {
        for (i in 1..distance.toInt()) {
            if (wlr.mc.theWorld.getBlockState(BlockPos(pos.x + lookVec.xCoord * i, pos.y - (if (player.onGround) 0.2 else 1.4), pos.z + lookVec.zCoord * i)).block == Blocks.air) {
                return true
            }
        }
        return false
    ...
    private fun barrierCheck(player: EntityPlayer, pos: BlockPos, distance: Float, lookVec: Vec3): Boolean {
        for (i in 1..distance.toInt()) {
            if (wlr.mc.theWorld.getBlockState(BlockPos(pos.x + lookVec.xCoord * i, pos.y - (if (player.onGround) 0.2 else 1.4), pos.z + lookVec.zCoord * i)).block == Blocks.barrier) {
                return true
            }
        }
        return false
