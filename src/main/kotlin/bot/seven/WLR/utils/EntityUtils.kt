...
object EntityUtils {
    ...
    fun getOpponentEntity(): EntityPlayer? {
        if (wlr.mc.theWorld != null) {
            for (entity in wlr.mc.theWorld.playerEntities) {
                if (entity.displayName != wlr.mc.thePlayer.displayName && shouldTarget(entity)) {
                    return entity
                }
            }
        }
    ...
    private fun shouldTarget(entity: EntityPlayer?): Boolean {
        return if (entity == null) {
            false
        } else if (wlr.mc.thePlayer.isEntityAlive && entity.isEntityAlive) {
            if (!entity.isInvisible /*&& !entity.isInvisibleToPlayer(mc.thePlayer)*/) {
                wlr.mc.thePlayer.getDistanceToEntity(entity) <= 64.0f
            } else {
                false
            }
        } else {
    ...
    private fun getClosestCorner(corner1: Vec3, corner2: Vec3, corner3: Vec3, corner4: Vec3): ArrayList<Vec3> {
        val pos = Vec3(wlr.mc.thePlayer.posX, wlr.mc.thePlayer.posY + wlr.mc.thePlayer.eyeHeight, wlr.mc.thePlayer.posZ)
